  ...
  if (submitted) {
    return (
      <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-8 text-center">
        <p className="text-emerald-200 font-medium">{mf.success}</p>
        <Link
          href="/map"
  ...
  return (
    <form
      ...
      onSubmit={async (e) => {
        ...
        try {
          const res = await fetch("/api/submit-media", {
            method: "POST",
    ...
    >
      <p className="text-sm text-white/55 leading-relaxed">{mf.intro}</p>
      ...
      <div>
        <label htmlFor="url" className="block text-sm font-medium text-white/80 mb-1.5">
          {mf.imageUrl} <span className="text-violet-400">*</span>
        </label>
        <p className="text-xs text-white/45 mb-2">{mf.imageUrlHint}</p>
        <input
          id="url"
          name="url"
          type="url"
      ...
      <PlaceOptionSelect
        ...
        onChange={setPlaceId}
        label={mf.location}
        unknownLabel={mf.locationUnknown}
        hint={mf.locationHint}
      />
